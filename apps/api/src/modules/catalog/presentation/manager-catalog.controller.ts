...
export class ManagerCatalogController {
  ...
  @Get('hotels/:id/rooms/operational')
  @Roles(Role.STAFF, Role.MANAGER, Role.ADMIN)
  @ApiOperation({
    summary: 'Get operational rooms with housekeeping & occupancy state',
  })
  getOperationalRooms(
    @Param() params: HotelIdParamsDto,
    @Req() req: AuthedRequest,
  ) {
    return this.manager.getOperationalRooms(params.id, req.user);
  }
  ...
  @Roles(Role.STAFF, Role.MANAGER, Role.ADMIN)
  @ApiOperation({ summary: 'Update room operational housekeeping status' })
  updateRoomStatus(
    @Param() params: RoomIdParamsDto,
    @Body() dto: UpdateRoomStatusDto,
    @Req() req: AuthedRequest,
  ) {
    return this.manager.updateRoomStatus(params.roomId, dto.status, req.user);
  }
