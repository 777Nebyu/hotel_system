...
export class ManagerCatalogService {
  ...
  async myHotels(actorOrUserId: string | CatalogActor) {
    if (typeof actorOrUserId === 'string') {
      return this.db.hotel.findMany({
        where: { managerId: actorOrUserId },
    ...
    if (actor.role === 'STAFF') {
      const hotelIds = await this.scope.getStaffHotelIds(actor.sub);
      return this.db.hotel.findMany({
  ...
  async updateRoomStatus(
    ...
    const room = await this.db.room.findUnique({
      where: { id: roomId },
      select: { id: true, hotelId: true, roomNumber: true, status: true },
    });
  ...
  async getOperationalRooms(hotelId: string, actor: CatalogActor) {
    await this.assertCanManage(hotelId, actor);
    ...
    return rooms.map((room) => {
      const activeDetail = room.details[0];
      const isOccupied = Boolean(activeDetail && activeDetail.booking);
      return {
        id: room.id,
        hotelId: room.hotelId,
        roomNumber: room.roomNumber,
        ...
        isOccupied,
        currentGuest: activeDetail?.booking?.user?.fullName ?? null,
        currentGuestPhone: activeDetail?.booking?.user?.phone ?? null,
        currentBookingId: activeDetail?.booking?.id ?? null,
        currentBookingRef: activeDetail?.booking?.bookingRef ?? null,
        checkInDate: activeDetail?.booking?.checkIn ?? null,
        checkOutDate: activeDetail?.booking?.checkOut ?? null,
        images: room.images,
